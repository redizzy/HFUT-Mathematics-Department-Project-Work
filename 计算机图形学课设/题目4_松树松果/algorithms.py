...
# ---------------------------------------------------------------------------
# 6. De Casteljau 算法 —— 任意阶 Bezier 曲线求值
# ---------------------------------------------------------------------------
def de_casteljau(control_points, t):
    """De Casteljau 递推求 Bezier 曲线在参数 t∈[0,1] 处的点。

    每一轮把相邻两个控制点按 (1-t):t 线性插值，得到一组少 1 个的新点；
    重复至只剩 1 点，即为曲线上 B(t)。
    对 n+1 个控制点（n 阶曲线）需 n 轮，O(n²)。

    参数：
        control_points: [(x, y), ...] 控制点列表（任意阶）
        t: 参数 ∈ [0, 1]
    返回：
        (x, y) 曲线上对应位置
    """
...
def bezier_curve(control_points, segments=24):
    """采样 Bezier 曲线，返回 segments+1 个均匀分布的点列。

    参数：
        control_points: 任意阶 Bezier 控制点
        segments: 采样段数（顶点数 = segments+1）
    """
